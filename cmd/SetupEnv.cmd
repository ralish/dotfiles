...
@REM with the CMD executable itself. This implies it's probably being called
@REM for interactive usage, rather than to run a batch script or program.
@REM
@REM FindStr would be better here as it can do a regular expression test, but
@REM it means launching a separate process, which we're trying to avoid.
