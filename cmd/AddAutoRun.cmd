...
@REM the large number of things that spawn a CMD process, but the bigger issue
@REM is the negative interactions it can have with other programs due to the
@REM changes it explicitly or implicitly makes to the CMD environment. Catching
@REM all these cases is effectively impossible, and they're often not obvious.

IF NOT DEFINED DOTFILES (
    ECHO The %%DOTFILES%% variable has not been set.
    EXIT /B
)

ECHO Setting HKCU\Software\Microsoft\Command Processor\AutoRun ...
