...
Function Global:Update-OpenSSHConfig {
    ...
    # OpenSSH outputs its version string on `stderr`. When redirecting to
    # `stdout`, Windows PowerShell creates an `ErrorRecord` for each line of
    # "error" output. While it's not a terminating error, custom prompts often
    # record the error count before and after command execution, with an
    # increase considered an indicator that the previous command failed. To
    # avoid this function erroneously being shown as having failed when used
    # with such prompts we clear the most recent error record if the error
    # count increased after retrieving the OpenSSH client version.
    try {
    ...
    try {
        try {
            # At least three lines to be considered a valid banner. The last
            # line is removed on the assumption it is a Vim modeline.
        ...
        try {
            ...
            foreach ($Include in $Includes) {
                # At least three lines to be considered a valid banner. The
                # last line is removed on the assumption it is a Vim modeline.
        ...
        # OpenSSH for Windows (which is *not* the same as OpenSSH Portable)
        # doesn't support the `sntrup761x25519-sha512@openssh.com` key exchange
        # algorithm. Until it does, we have to remove any usage of it in our
        # configuration template or OpenSSH complains about an unsupported KEX
        # algorithm. This hack is written to ensure we don't remove it when/if
        # support is added.
            ...
            if ([Version]$Version -lt $DirectivesVersion) {
                ...
                foreach ($Directive in $NewDirectives[$DirectivesVersion]) {
                    ...
                    for ($i = 0; $i -lt $ConfigOld.Count; $i++) {
                        ...
                        # Matched a line to be excluded; walk backwards to
                        # remove comments and new lines which pertain to the
                        # directive.
